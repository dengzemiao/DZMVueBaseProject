  ...
  <div class="menu-custom" :data-path="data.path" @click="touchRoute">
    ...
    <template v-else>
      ...
      <template v-if="data.meta.sicon">
        <!-- 选中路由地址 -->
        <template v-if="data.path === selectPath">
          <a-icon :type="data.meta.sicon" />
        </template>
        <!-- 未选中路由地址 -->
        <template v-else>
          <a-icon v-if="data.meta.icon && data.path !== hoverPath" :type="data.meta.icon" />
          <a-icon v-if="data.meta.sicon && data.path === hoverPath" :type="data.meta.sicon" />
        </template>
      </template>
      <!-- 无选中 icon -->
      <template v-else>
        <a-icon v-if="data.meta.icon" :type="data.meta.icon" />
      </template>
    </template>
    <!-- 标题 -->
